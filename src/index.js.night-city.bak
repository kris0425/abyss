...
const { actionButtons, buffMenu, classMenu, combatButtons, gameEmbed, gameFiles, hiddenRoomButtons, inventoryMenu, shopMenu } = require("./components");
...
async function handleBuffSelect(interaction) {
  ...
  const player = createPlayer(interaction.user.id, classKey, buffKey);
  setPlayer(player);
